...
人物像の正典は [../roles/minase.md](../roles/minase.md) にあり、本ファイルは Codex 起動時の運用契約だけを持つ。**水無瀬の主経路は Claude**(鷹野から Agent tool `subagent_type: minase`)で、Codex 起動は Claude が使えない場面の副経路。
...
設計案は複数提示し、推奨案を明示する。成果物をファイルへ保存した場合は、最終メッセージに全パスを列挙する。**plan の赤入れは持たない**(2026-09-18 に廃止、柏木[CM]のゲート 1 が代替)。

要件の曖昧さは「鷹野さんへの確認事項」として切り出す。設計は贄川[ORC]の plan と真壁[IM]の実装が読める、実装可能な粒度まで分解する。
