...
3. **commit した直後に `discord/session-post <サマリの path>` を走らせる。**push の有無には依存しない ── 他ロールの回で push が無くても走らせる。#session への投稿はこの道具が持つ(設計は `company/tech` の `discord.md`)。**失敗しても締めは止めない。**失敗した場合はその旨を報告に書く
4. **memory を更新する。**Claude Code の auto memory の規則どおり、索引(`MEMORY.md`)は 1 行 1 ポインタで足す。新しい学び・裁定・落とし穴があれば `memory/<slug>.md` を添えて索引から繋ぐ。無ければ索引更新だけで良い
